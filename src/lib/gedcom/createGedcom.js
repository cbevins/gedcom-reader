/**
 * Creates a searchable Gedcom instance from _gedcomData.js data.
...
export function createGedcom(_gedcomData) {
    function _isKey(field) { return field[0] === '@' }
    const gedcom = new Gedcom()
    const messages = []
    for(let i=0; i<_gedcomData.length; i++) {
        const line = _gedcomData[i]
        const lineNo = i+1
